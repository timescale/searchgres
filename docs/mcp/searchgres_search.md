...
Runs semantic, full-text, hybrid, or filter-only search with the recursive structured filter object. `select` projects complete RPC results locally using the same selectors as `searchgres search --select`.
